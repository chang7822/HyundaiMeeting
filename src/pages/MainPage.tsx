...
import { matchingApi, chatApi, authApi, companyApi, noticeApi, pushApi, notificationApi, extraMatchingApi, starApi, adminApi, systemApi } from '../services/api';
...
const MainPage = ({ sidebarOpen }: { sidebarOpen: boolean }) => {
  ...
  // 마운트 시 하단 배너 광고 숨김 (네이티브 앱)
  useEffect(() => {
    if (!Capacitor.isNativePlatform()) return;
    if ((window as any).globalBannerAd && (window as any).globalBannerShowing) {
      (window as any).globalBannerShowing = false;
      (window as any).globalBannerAd.hide?.().catch(() => {});
    }
  }, []);
  ...
  const handleMatchingStarConfirm = async () => {
    ...
    // 광고 OFF 또는 네이티브 앱이 아닌 경우 바로 매칭 신청
    ...
    try {
      ...
      const isAdBlocked = errStr.includes('net::ERR') || errStr.includes('Failed to fetch') || errStr.includes('Network') || errStr.includes('blocked');
      const isNoFill = errStr.includes('no fill') || errStr.includes('No fill') || errStr.includes('ERROR_CODE_NO_FILL') || errStr.toLowerCase().includes('no ad');
      if (isAdBlocked) {
