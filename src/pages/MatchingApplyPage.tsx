...
/** 프로필 모달 전용 - 모바일에서 여백·폭 조정 */
const ProfileModalContent = styled(ModalContent)`
...
const MatchingApplyPage = ({ sidebarOpen }: { sidebarOpen: boolean }) => {
  ...
  // MatchingApplyPage 마운트 시 LoadingSpinner의 전역 배너 즉시 숨김
  useEffect(() => {
    ...
    if (window.globalBannerAd && window.globalBannerShowing) {
      ...
      window.globalBannerAd.hide()
        .then(() => {
          // console.log('[MatchingApplyPage] 전역 배너 숨김 완료');
        })
        .catch((error: any) => {
          console.error('[MatchingApplyPage] 전역 배너 숨김 실패:', error);
        });
    }
  }, []);
      ...
      try {
        ...
        const [periodData, noticeData] = await Promise.all([
          matchingApi.getMatchingPeriod().catch(err => {
            console.error('[MatchingApplyPage] 매칭 기간 API 에러:', err);
            return null;
          }),
          noticeApi.getNotices().catch(err => {
            console.error('[MatchingApplyPage] 최신 공지사항 조회 오류:', err);
            return null;
          })
        ]);
        ...
        console.error('[MatchingApplyPage] 초기 데이터 로드 실패:', error);
        // 에러 발생해도 페이지는 표시
        setLoadingPeriod(false);
        setIsLoadingNotice(false);
        setIsPageReady(true);
  ...
  useEffect(() => {
    ...
    const fetchExtraMatchingFeature = async () => {
      ...
      try {
        ...
        console.error('[MatchingApplyPage] 추가 매칭 도전 기능 설정 조회 오류:', e);
  ...
  useEffect(() => {
    ...
    const fetchCommunitySettings = async () => {
      try {
        ...
        console.error('[MatchingApplyPage] 커뮤니티 설정 조회 오류:', e);
        setCommunityEnabled(true);
      }
    };
  ...
  const checkUserBanStatus = useCallback(async () => {
    try {
      ...
      console.error('[MatchingApplyPage] 사용자 상태 확인 오류:', error);
    }
  }, [user?.is_banned, user?.banned_until, fetchUser]);

  // MatchingApplyPage 진입 시 정지 상태 확인 후 매칭 상태 로드 (초기 로딩에 포함)
  useEffect(() => {
  ...
  const fetchPartnerProfile = async (partnerUserId: string) => {
    ...
    try {
      ...
      console.error('[MatchingApplyPage][fetchPartnerProfile] API 실패:', e);
  ...
  useEffect(() => {
    ...
    const pollStatus = async () => {
      try {
        ...
        console.error('[MatchingApplyPage] 매칭 기간 자동 갱신 오류:', err);
  ...
  const handleMatchingRequest = async () => {
    ...
    try {
      ...
      console.error('[MatchingApplyPage] 프로필 조회 중 오류:', e);
      bodyTypes = [];
    }
    ...
    if (me) {
      try {
        ...
        console.error('[MatchingApplyPage] setProfile 중 오류:', e);
      }
    }
  ...
  if (isBanned && !isBanExpired) {
    ...
    return (
      <MainContainer $sidebarOpen={sidebarOpen}>
        <TopHeaderRow>
          <TopWelcomeTitle style={{ marginBottom: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <FaHandshake style={{ color: 'inherit', fontSize: '1.2em' }} />
            매칭 신청
          </TopWelcomeTitle>
        </TopHeaderRow>

        {communityEnabled && (
          <div style={{ background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(124,58,237,0.15)', borderRadius: 10, padding: '10px 14px', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, boxShadow: '0 2px 6px rgba(0,0,0,0.05)' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ color: '#7C3AED', fontSize: '0.75rem', fontWeight: 500, margin: 0 }}>💬 회원들과 익명으로 자유롭게 소통해보세요</p>
              <p style={{ color: '#9ca3af', fontSize: '0.7rem', margin: '2px 0 0 0' }}>커뮤니티에서 익명으로 자유롭게 소통해보세요</p>
            </div>
            <button onClick={() => navigate('/main')} style={{ position: 'relative', background: 'linear-gradient(135deg,#7C3AED,#5b21b6)', color: 'white', border: 'none', padding: '6px 12px', borderRadius: 8, fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}><NewBadge>NEW!</NewBadge> 커뮤니티</button>
          </div>
        )}
        <div style={{ background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(124,58,237,0.15)', borderRadius: 10, padding: '10px 14px', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, boxShadow: '0 2px 6px rgba(0,0,0,0.05)' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ color: '#7C3AED', fontSize: '0.75rem', fontWeight: 500, margin: 0 }}>⭐ 별이 모자라다구? 가위바위보는 할 줄 알지?</p>
            <p style={{ color: '#9ca3af', fontSize: '0.7rem', margin: '2px 0 0 0' }}>하루 세판 가위바위보를 해서 별을 더 획득해보세요!</p>
          </div>
          <button onClick={() => navigate('/rps-arena')} style={{ background: 'linear-gradient(135deg,#667eea,#764ba2)', color: 'white', border: 'none', padding: '6px 12px', borderRadius: 8, fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer', flexShrink: 0, overflow: 'visible' }}>
    ...
    try {
      ...
      if (me) {
        try {
          ...
          console.error('[MatchingApplyPage] 프로필 모달 setProfile 중 오류:', e);
        }
      }
    } catch (e) {
      console.error('[MatchingApplyPage] 프로필 모달용 프로필 조회 중 오류:', e);
    } finally {
  ...
  return (
    <MainContainer $sidebarOpen={sidebarOpen}>
      <TopHeaderRow>
        <TopWelcomeTitle style={{marginLeft: 10, marginBottom: 20, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <FaHandshake style={{ color: 'inherit', fontSize: '1.2em' }} />
          매칭 신청
        </TopWelcomeTitle>
        <ProfileHeaderButton
        ...
        {communityEnabled && (
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ color: '#7C3AED', fontSize: '0.75rem', fontWeight: '500', margin: 0, lineHeight: '1.3' }}>
                💬 매칭신청까지 심심하지?
              </p>
              <p style={{ color: '#9ca3af', fontSize: '0.7rem', margin: '2px 0 0 0', lineHeight: '1.2' }}>
                ...
                ? (() => {
                    ...
                    return (
                      <StatusChatWrapper>
                        ...
                        {/* 회색 박스 - 매칭 성공/실패 시 맨트·일정은 위 매칭신청 위젯에 이미 있으므로 생략 */}
                        <div
                          style={{
                            marginTop: 0,
                            background:
                            ...
                            display: 'flex',
                            flexDirection: 'column',
                            justifyContent: (period?.startsWith('신청기간 :') || status === '신청 완료' || status === '매칭 실패' || status === '매칭 성공') ? 'center' : undefined,
                            minHeight: (period?.startsWith('신청기간 :') || status === '신청 완료' || status === '매칭 실패' || status === '매칭 성공') ? 56 : undefined,
                          }}
                        >
                          <div
                            style={{
                              ...
                              fontSize: '1.2rem',
                              color,
                              marginBottom: (period?.startsWith('신청기간 :') || status === '신청 완료' || status === '매칭 실패' || status === '매칭 성공') ? 0 : (period ? 8 : 0),
                              lineHeight: 1.3,
                              textAlign: 'center',
                            }}
                          >
                          ...
                          {(period?.startsWith('신청기간 :') || status === '신청 완료' || status === '매칭 실패' || status === '매칭 성공') ? null : (period || hasScheduleData) && (
                            <div
                              style={{
                                display: 'flex',
                                alignItems: 'center',
      ...
      {/* 컴팩트 2x2 그리드 카드들 (공지사항/FAQ/프로필/선호스타일) - 숨김 */}
      {false && <CompactGrid>
...
export default MatchingApplyPage; 