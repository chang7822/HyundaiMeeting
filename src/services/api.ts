...
export const extraMatchingApi = {
  ...
  // "저를 추천합니다" 엔트리 생성 (별 10개 사용)
  createEntry: async (): Promise<any> => {
    const response = await api.post('/extra-matching/entries');
    return response.data;
  },
  ...
  // 특정 엔트리에 "저는 어때요" 신청 (별 10개 사용)
  applyEntry: async (entryId: number): Promise<any> => {
    const response = await api.post(`/extra-matching/entries/${entryId}/apply`);
    return response.data;
  },
